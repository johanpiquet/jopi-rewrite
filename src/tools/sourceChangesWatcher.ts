...
export class SourceChangesWatcher {
    ...
    async start() {
        ...
        if (sourceDir) this.addWatchDir(sourceDir);
     ...
     */
    async detectSourceDir(cwd: string = process.cwd()): Promise<string | null> {
        ...
        try {
            ...
            const rootDir: string | undefined = tsconfig?.compilerOptions?.rootDir;
            if (rootDir) {
        ...
        try {
            ...
            const hinted: string | undefined = pkg.source || pkg?.directories?.src;
            if (hinted) {
