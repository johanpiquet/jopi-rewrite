...
> **ATTENTION** Le système automatique de cache, anonymise l'utilisateur.
> Ses rôles sont vérifiés lors de l'accès à la page, puis l'utilisateur est rendu anonyme
> afin que la page mise en cache soit générique.
...
La fonction `req.fakeNoUser` permet d'anonymiser l'utilisateur, en faisant comme si aucun utilisateur n'est connecté.
Cette fonction est appelée lorsque le cache automatique est activé, et qu'il est nécessaire de générer le contenu à mettre en cache.

Le comportement de la fonction `req.fakeNoUser` peut-être modifié, en ajoutant un écouteur via la fonction `req.onFakeNoUser`.
Cet écouteur peut annuler l'appel à `req.fakeNoUser`, ou ajouter des modifications (ex: suppression de cookies).

Cet écouteur peut être positionné:
* Via `cache_beforeAddToCache` dans le fichier `config.ts`de la route.
* Via `configure_cache().on_fakeNoUser` dans le fichier `src/index.ts` ou le fichier `serverInit.ts` d'un module.