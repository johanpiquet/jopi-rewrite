...
The function `req.fakeNoUser` anonymizes the user, acting as if no user is logged in.
This function is called when automatic cache is enabled and it is necessary to generate the content to be cached.

The behavior of `req.fakeNoUser` can be modified by adding a listener via `req.onFakeNoUser`.
This listener can cancel the call to `req.fakeNoUser`, or add modifications (e.g. removing cookies).
