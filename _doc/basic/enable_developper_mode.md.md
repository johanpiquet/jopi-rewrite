...
When you use a project template to create your Jopi Rewrite project (npx jopi init) the `package.json` file exposes two scripts:
...
* `jopib` starts the Bun.js version of the application, while its equivalent `jopin` starts the Node.js version. These two tools simply avoid typing `--preload jopi-rewrite/loader --preload jopi-rewrite` in the Node/Bun execution parameters.
